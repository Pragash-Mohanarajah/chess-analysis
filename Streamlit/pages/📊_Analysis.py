...
class Analysis:
    ...
    def games_won(self):
        return (len(self.data[(self.data.white_username == "PragashMohanarajah") & (self.data.white_result == "win")]) + len(self.data[(self.data.black_username == "PragashMohanarajah") & (self.data.black_result == "win")])) / self.total_games() * 100
    
    def games_lost(self):
        return 100 - self.games_won() 
    
    def games_with_white(self):
        return (len(self.data[(self.data.white_username == "PragashMohanarajah")])) / self.total_games() * 100
    ...
    def peak_rating(self):
        return max(max(self.data[self.data.white_username == "PragashMohanarajah"]["white_rating"]),max(self.data[self.data.black_username == "PragashMohanarajah"]["black_rating"]))
    ...
    def highest_white_opp_win(self):
        data = self.data[(self.data["white_username"] == "PragashMohanarajah") & (self.data['white_result'] == "win")]
        return data[data.black_rating == max(data.black_rating)][["black_rating", "black_username"]]
    
    def highest_black_opp_win(self):
        data = self.data[(self.data["black_username"] == "PragashMohanarajah") & (self.data['black_result'] == "win")]
    ...
    def add_column_my_rating(self) -> list:
        ...
        for i in range(len(self.data)):
            ...
            if row.white_username == "PragashMohanarajah":
